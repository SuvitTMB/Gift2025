    ...
    snapshot.forEach(doc=> {
      ...
      if(xCheck!=doc.data().StockGroup==1) {
        ...
        if(doc.data().StockGroup==1) {
          ...
          if(p1==0) {
            str += '<center><div class="btn-leaderboard">ชุดปฏิทินปีใหม่ 2568</div></center>';
            p1 = 1;
          } 
        } else if(doc.data().StockGroup==2) {
          str += '<div id="Point2"></div>';
          if(p2==0) {
            str += '<center><div class="btn-leaderboard">ของขวัญที่มีมูลค่าตั้งแต่ 0 - 70 บาท</div></center>';
            p2 = 1;
          } 
        } else if(doc.data().StockGroup==3) {
          str += '<div id="Point3"></div>';
          if(p3==0) {
            str += '<center><div class="btn-leaderboard">ของขวัญที่มีมูลค่าตั้งแต่ 71 - 200 บาท</div></center>';
            p3 = 1;
          } 
        } else if(doc.data().StockGroup==4) {
          str += '<div id="Point4"></div>';
          if(p4==0) {
            str += '<center><div class="btn-leaderboard">ของขวัญที่มีมูลค่าตั้งแต่ 200 บาทขึ้นไป</div></center>';
            p4 = 1;
          } 
        }
      //} else {
